...
# Add parent directory to sys.path so we can import module2 from the root
        ...
        try:
            ...
            with httpx.Client(timeout=30.0) as client:
                ...
                if res.status_code != 200:
                    raise Exception(f"RunPod trigger failed (HTTP {res.status_code}): {res.text}")
                job_data = res.json()
            
            job_id = job_data.get("id")
            status = job_data.get("status")
            self._log(f"Job triggered successfully on RunPod. Job ID: {job_id}")
            self._log(f"RunPod Job status: {status}")
            
            # 2. Poll RunPod job status and stream progress
            ...
            while True:
                ...
                with httpx.Client(timeout=30.0) as client:
                    ...
                    if res.status_code != 200:
                        self._log(f"Warning: Failed to poll RunPod status: {res.text}")
                        continue
                    job_status = res.json()
                
                curr_status = job_status.get("status")
                
                # Check for state changes or final outcomes
                if curr_status != last_status:
                    self._log(f"RunPod Job status changed to: {curr_status}")
                    last_status = curr_status
                
                if curr_status == "COMPLETED":
                    self._emit({"type": "status", "stage": "done", "detail": "RunPod job complete."})
                    output = job_status.get("output", {})
                    clip_mappings = output.get("clip_mappings", {})
                    
                    # Generate a mock manifest.json in the local output folder so module2.result endpoint
                    # can read the completion state side-by-side with local recordings!
                    out_dir = os.path.join(OUTPUT_ROOT, self.session_id, "module2", self.model_name)
                    os.makedirs(out_dir, exist_ok=True)
                    
                    # Download files from clip mappings to local out_dir so the local frontend player plays them!
                    self._log("Downloading cloned audio clips from Supabase to local output folder for playback...")
                    clips = []
                    for filename, signed_url in clip_mappings.items():
                        ...
                        sentence_id = parts[0] if len(parts) > 0 else "unknown"
                        style_id = parts[1].replace(".wav", "") if len(parts) > 1 else "normal"
                        ...
                        clips.append({
                            ...
                            "style_id": style_id,
                            "style_applied": True,
                            "text": "Cloned audio downloaded from RunPod Serverless.",
                            "duration_sec": 3.0,
                            "gen_sec": 1.0,
                            "rtf": 0.3
                        })
                    ...
                    manifest = {
                        "model": self.model_name,
                        "device": output.get("device", "RunPod GPU"),
                        "sample_rate": output.get("sample_rate", 24000),
                        "reference": {
                            "calm_tag": "normal",
                            "expressive_tag": "happy",
                            "reference_used": "normal"
                        },
                        "clips": clips
                    }
                    ...
                    self.active = False
                    self._emit({"type": "result", "manifest": manifest})
                    self._emit({"type": "done", "status": "done", "message": "complete"})
                    break
                    
                elif curr_status in ("FAILED", "CANCELLED"):
                    err_msg = job_status.get("error", "Unknown error occurred on RunPod.")
                    raise Exception(f"RunPod execution failed: {err_msg}")
                    
        except Exception as e:
            self.active = False
            self._log(f"RunPod Execution Error: {e}")
...
def _maybe_autoclone(session_id: str, output_dir: str, result: dict | None) -> dict | None:
    ...
    if RUNPOD_ENDPOINT_ID and RUNPOD_API_KEY:
        cloning_run = RunPodCloningRun(output_dir, DEFAULT_MODEL)
    else:
...
async def module2_run(body: Module2RunRequest):
    ...
    if RUNPOD_ENDPOINT_ID and RUNPOD_API_KEY:
        cloning_run = RunPodCloningRun(session_dir, model)
    else:
